...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def init_layers(self):
        ...
        # self.reference_points = nn.Linear(self.embed_dims, 20*2) #instance level
        self.reference_points = nn.Linear(self.embed_dims, 2) #TODO ident init
        ...
        self.bev_downsample = PatchMerging((200,100),self.embed_dims)
    ...
    def forward(self,
        ...
        # reference_points = self.reference_points(query_pos).reshape(bs,num_query,20,2)
        ...
        reference_points = reference_points.sigmoid()
        reference_points = reference_points.repeat(1, 1, 20)
        bs, num_qeury, _ = reference_points.shape
        reference_points = reference_points.view(bs, num_qeury, 20, 2)
        ...
        # import ipdb; ipdb.set_trace()
        #TODO bev_embbed downsampled 
        bev_embed_downsample=self.bev_downsample(bev_embed.permute(1, 0, 2)).permute(1,0,2)
        bev_input=torch.cat([bev_embed, bev_embed_downsample],dim=0)


        inter_states, inter_references,inter_states_instance,inter_MGA_Ref = self.decoder(
            query=query,
            key=None,
            # value=bev_embed,
            value=bev_input,
            ...
            spatial_shapes=torch.tensor([[bev_h, bev_w],[bev_h//2, bev_w//2]], device=query.device),
            level_start_index=torch.tensor([0,bev_h*bev_w], device=query.device),
            mlvl_feats=mlvl_feats,
        ...
        # return bev_embed, depth, inter_states, init_reference_out, inter_references_out,inter_states_instance,inter_MGA_Ref

        return bev_embed, depth, inter_states, init_reference_out, inter_references_out,inter_states_instance,inter_MGA_Ref,bev_embed_downsample
